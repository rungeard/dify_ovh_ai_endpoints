    ...
    def _invoke(self, model: str, credentials: dict, text: str, user: str | None = None) -> bool:
        ...
        if response.status_code != HTTPStatus.OK:
            raise InvokeBadRequestError(
                format_ovh_rate_limit_error(response.status_code, response.text)
            )
        ...
        try:
            ...
            raise InvokeBadRequestError(
                f"Invalid OVH moderation response: {response.text[:1000]}"
            ) from ex

        match = _SAFETY_LABEL_PATTERN.search(content)
        if not match:
            raise InvokeBadRequestError(
                f"Unable to parse safety label from OVH moderation output: {content[:500]}"
            )
